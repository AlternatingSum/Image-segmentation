...
# -*- coding: utf-8 -*-
"""
Uses a U-Net to refine a predicted segmentation mask
...
unet = load_model('../data/model_weights/unet_16filters_32_dist.h5')
...
def create_new_mask(prefix):
    ...
    correct_mask_file = "../data/test/correct_masks/" + prefix + "_mask.png"
    
    # Processes the image and adds padding
    motorcycle = process.Masked_Image(image_file, tuned_prediction_file, tuned_prediction_file)
    ...
    # Modifies the prediction in a region around each boundary point
    for boundary_point in boundary_list:
        row = boundary_point[0]
        col = boundary_point[1]
        
        # Uses the u-net to make a prediction in the region around the boundary point
        region = motorcycle.find_subregion(row, col, buffer)
        local_prediction = unet.predict(np.expand_dims((region[:,:,:4]), axis=0))
        
        # Adds the weighted prediction to the first entry (in dimension = 2)
        prediction_array[row-buffer:row+buffer, col-buffer:col+buffer,0] += local_prediction[0,:,:,0] * gaussian
        
        # Adds the weight to the second entry (in dimension = 2)
    ...
    # Blurs the initial predicted foreground, so there aren't sharp changes in its predictions
    motorcycle.predicted_foreground = skimage.filters.gaussian(motorcycle.predicted_foreground, sigma = sigma, multichannel=False)
    
    # Incorporates the initial predicted foreground into the prediction array
    prediction_array[:,:,0] += motorcycle.predicted_foreground
    ...
    print("IoU for the first predicted mask:")
    print(find_iou(correct_mask_file, first_predicted_mask))
    print("IoU for the fine tuned mask:")
    print(find_iou(correct_mask_file, tuned_predicted_mask))
    print("IoU for the new mask, created by the u-net:")
...
current_prefix = "t_41"
create_new_mask(current_prefix)
