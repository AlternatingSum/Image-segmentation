...
# -*- coding: utf-8 -*-
"""
Open_Images_pre_processing.py

Image pre-processing for foreground detection. Given an image this program can resize, pad, and blur it. 
The program also calculates the first and second derivatives of the colors in an image, and creates
a dataset used to train a specialist network for the imag. 
"""
...
class Masked_Image:
    ...
    def __init__(self, image_file, foreground_file):
        ...
        self.foreground = skimage.transform.resize(self.foreground, (self.height, self.width))
        if np.amax(self.foreground) > 1.0:
            self.foreground = self.foreground/np.amax(self.foreground)
        self.foreground = np.rint(self.foreground)
    ...
    def add_padding(self, h_buffer, v_buffer):
        ...
        left_image_padding = self.image[:, :1, :]*np.ones((1, math.floor(h_buffer), 3))
        right_image_padding = self.image[:, -1:, :]*np.ones((1, math.ceil(h_buffer), 3))

        self.image = np.concatenate((left_image_padding, np.concatenate((self.image, right_image_padding), axis=1)), axis=1)
        
        top_image_padding = self.image[:1, :, :]*np.ones((math.floor(v_buffer), 1, 3))
        bottom_image_padding = self.image[-1:, :, :]*np.ones((math.ceil(v_buffer), 1, 3))
        ...
        left_foreground_padding = self.foreground[:, :1]*np.ones((1, math.floor(h_buffer)))
        right_foreground_padding = self.foreground[:, -1:]*np.ones((1, math.ceil(h_buffer)))
        ...
        top_foreground_padding = self.foreground[:1, :]*np.ones((math.floor(v_buffer), 1))
        bottom_foreground_padding = self.foreground[-1:, :]*np.ones((math.ceil(v_buffer), 1))
    ...
    def find_boundary(self):
        ...
        self.foreground = np.reshape(self.foreground[:,:], (self.height, self.width))
    ...
    def create_sample_set(self):
        ...
        for row in range(self.height-2*buffer):
            for col in range(self.width-2*buffer):
                ...
                elif cropped_foreground[row][col] == 1.0:
        ...
        return (foreground_samples, background_samples, boundary_samples, all_samples)
    
    
    def create_train_and_cv(self, foreground_samples, background_samples, boundary_samples, all_samples):
        ...
        return (train_inputs, train_outputs, cv_inputs, cv_outputs, all_samples)
    ...
    def process_image(self):
        ...
        self.resize(500)
        self.add_padding(2, 2)
        original_image = np.copy(self.image)
        
        self.smooth_image(12)
        
        samples = self.create_sample_set()
        
        return (original_image, self.create_train_and_cv(samples[0], samples[1], samples[2], samples[3]))
    
    def process_image_input_only(self):
        """Resizes the image and adds padding."""
        
        self.resize(500)
        self.add_padding((504-self.width)/2, (504-self.height)/2)
